...
void update_tree(ll node, ll a, ll b, ll i, ll j, ll value, ll opr) {
	if(lazy[node].first==3) {
		...
		if(a != b) {
			...
			else lazy[node*2].second = (lazy[node*2].second*lazy[node].second)%md;
			if(lazy[node*2+1].first==0) {
				lazy[node*2+1].first = lazy[node].first;
				lazy[node*2+1].second = lazy[node].second;
			}
			else lazy[node*2+1].second = (lazy[node*2+1].second*lazy[node].second)%md;
		}
	}
	lazy[node].second=0;
	lazy[node].first=0;
  	...
  	if(a >= i && b <= j) {
  		if(opr==3) {
			...
			if(a != b) {
				...
				else lazy[node*2].second = (lazy[node*2].second*value)%md;
				if(lazy[node*2+1].first==0) {
					lazy[node*2+1].first = 2;
					lazy[node*2+1].second = value;
				}
				else lazy[node*2+1].second = (lazy[node*2+1].second*value)%md;
			}
		}
		return;
	}
